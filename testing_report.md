...
#### 1. Проблема: Блокировка загрузки ленты контента
* **Описание:** Лента контента не загружается, отображается уведомление «Завершите настройку Яндекса».
* **Примечание:** Ошибка может быть вызвана как невыполненными настройками профиля, так и блокировкой сетевых запросов расширениями (ошибка `net::ERR_BLOCKED_BY_CLIENT` в консоли).
* **Статус:** Требует проверки на чистом профиле (без расширений) для подтверждения связи с «настройкой».
